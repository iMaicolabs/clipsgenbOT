    ...
    try {
      ...
      pollRef.current = setInterval(async () => {
        ...
        if (s?.hasToken) {
          ...
          setMsg({ type: "ok", text: "✅ Cuenta de YouTube conectada correctamente." });
          await refreshStatus();
      ...
      {status && (
        <div className={`rounded-lg p-4 mb-6 text-sm ${isAuthenticated ? "bg-zinc-800 border border-zinc-700" : "bg-red-950 border border-red-800"}`}>
          {isAuthenticated ? (
            ...
            <p className="text-red-400 font-semibold">⚠️ Sin autenticación — las descargas pueden fallar</p>
          )}
        </div>
      )}
      ...
      {tab === "oauth" && (
        <div>
          <p className="text-sm text-zinc-400 mb-4">
            Conecta una cuenta de Google/YouTube directamente. El token dura meses sin expirar.
            Si las descargas siguen fallando tras conectar, usa también las <button onClick={() => setTab("cookies")} className="text-blue-400 underline">cookies manuales</button>.
          </p>
              ...
              <ol className="text-sm space-y-3">
                ...
                <li className="flex gap-3">
                  ...
                  <span className="text-zinc-400">
                    Esta página se actualizará automáticamente cuando se complete.
                  </span>
                </li>
              </ol>
        ...
        <div>
          <p className="text-sm text-zinc-400 mb-3">
            ...
            <a href="https://chromewebstore.google.com/detail/get-cookiestxt-locally/cclelndahbckbenkjhflpdbgdldlbecc"
               target="_blank" rel="noreferrer" className="text-blue-400 underline">
              Get cookies.txt LOCALLY
            </a>{" "}
            y pégalas aquí. Renuévalas cada 2–4 semanas si las descargas vuelven a fallar.
          </p>
